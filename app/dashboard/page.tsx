...
import { MonthlyBarChart, DonutChart } from '@/components/charts/Charts'
import {
  mockAnalytics,
  formatCurrency,
} from '@/lib/mockData'
import { Wallet, Users, ArrowLeftRight, ShieldAlert, Clock, TrendingUp } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  if (loading || !data) return <AppLayout><div className="p-6 text-white">Loading...</div></AppLayout>
      ...
      <div className="w-full flex flex-col gap-6 p-6 animate-fade-in">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
          ...
          <div className="lg:col-span-1 card p-6 flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display font-600 text-[14px] text-white">Active Alerts (Fraud)</h3>
            ...
            <div className="divide-y divide-[#1a1d24] flex-1 overflow-y-auto min-h-0">
              {data.fraudAlerts.map((alert: any) => (
                <div key={alert.transaction_id} className="py-3 first:pt-0 last:pb-0 w-full">
                  <div className="flex items-start gap-3 w-full">
                    <span className={`mt-1.5 w-2 h-2 rounded-full flex-shrink-0 bg-danger`} />
                    <div className="flex-1">
                      <p className="text-[13px] text-[#e8eaf0] leading-snug">Suspicious activity detected on {alert.account_number} for {formatCurrency(parseFloat(alert.amount))}</p>
                      <p className="text-[11px] text-[#8890a0] mt-1.5">{new Date(alert.created_at).toLocaleDateString('en-IN')}</p>
                    </div>
                  </div>
                </div>
              ))}
              {data.fraudAlerts.length === 0 && (
                <div className="py-4 text-center text-[13px] text-[#8890a0]">No active alerts</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="card w-full">
          <div className="flex items-center justify-between px-6 py-5 border-b border-[#1a1d24]">
            ...
            <a href="/transactions" className="text-[13px] text-accent hover:underline font-medium">View all &rarr;</a>
          </div>
          <div className="divide-y divide-[#1a1d24] w-full block">
            {data.recentTx.map((tx: any) => (
              <div key={tx.id} className="flex w-full items-center justify-between gap-4 px-6 py-4 hover:bg-[#12141a] transition-colors">
                <div className="flex items-center gap-4 w-1/3 min-w-0">
                  <div className="min-w-0 flex-1">
                    <p className="text-[14px] font-medium text-white truncate w-full">{tx.description}</p>
                ...
                <div className="w-1/4">
                  ...
                  <p className="text-[12px] text-[#8890a0] mt-0.5">{new Date(tx.transaction_date).toLocaleTimeString('en-IN', {hour: '2-digit', minute:'2-digit'})}</p>
                </div>

                <div className="w-1/4">
                  {txBadge(tx.type)}
                  {tx.is_suspicious && <span className="ml-3 w-2 h-2 inline-block rounded-full bg-danger flex-shrink-0" title="Suspicious" />}
                </div>
